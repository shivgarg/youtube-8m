...
class YT8MFrameFeatureReaderNoPCA(BaseReader):
  """Reads TFRecords of SequenceExamples.
  ...
  def __init__(self,
    ...
    self.feature_names = feature_names
    self.max_frames = max_frames
    self.pca_eigenvals = np.load(open('pca_weights/eigenvals.npy','r'))[:1024]
    self.pca_eigenvecs = np.load(open('pca_weights/eigenvecs.npy','r'))[:,:1024]
    self.pca_mean = np.load(open('pca_weights/mean.npy','r'))
  ...
  def get_video_matrix(self,
    ...
    if feature_size == 1024:
      feature_matrix = feature_matrix*tf.transpose(tf.sqrt(self.pca_eigenvals + 1e-4))
      feature_matrix = tf.reduce_sum(tf.multiply(tf.expand_dims(feature_matrix,1),self.pca_eigenvecs),2)
